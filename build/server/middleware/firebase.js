...
module.exports = () => async (req, res, next) => {
  ...
  // Detecting if a path is in whitelist
  ...
  const modelPluralName = req.url.split('/')[1]
  ...
  if (!authorization) {
    ...
    let roleInstance
    try {
      roleInstance = await req.app.models.MyRole.findOne({
        where: {
          code: anonymousCode
        }
      })
  ...
  let roleIds
  try {
    roleIds = await req.app.models.AssignedRole.find({where: {userId: userInstance.id}})
    roleIds = roleIds ? roleIds.map(item => item.roleId) : undefined
  } catch (error) {
    throw error
  }
  ...
  try {
    ...
    return res.status(401).send('permission does not exists')
  }
